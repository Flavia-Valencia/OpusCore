...
$stmt = $conexion->prepare("
    SELECT
        ...
        sol.idEstudiante    AS usuario_id,
        ...
        rn.notaFinal,
        rn.estadoEstudiante,
        rn.fechaRegistro    AS fechaActividad,
        est.id              AS estudiante_id
    FROM solicitudConstanciaEstudiante sol
    INNER JOIN usuarios u        ON sol.idEstudiante = u.id
    INNER JOIN cursos c          ON sol.idCurso = c.id
    LEFT  JOIN PeriodoInscripcion pi ON c.idPeriodo = pi.id
    LEFT  JOIN estudiantes est   ON est.usuario_id = u.id
    LEFT  JOIN RegistroNotas rn  ON rn.idEstudiante = est.id AND rn.idCurso = c.id
...
// 2. Validar datos completos del usuario (HU-28)
if (empty($sol['nombre']) || empty($sol['apellido']) || empty($sol['correo'])) {
    echo json_encode(['error' => true, 'mensaje' => 'El usuario no tiene información completa (nombre, apellido o correo)']);
    exit();
}
...
// 3. Validar que el estudiante tenga el curso aprobado (HU-30)
if (empty($sol['notaFinal']) || $sol['estadoEstudiante'] !== 'Aprobado') {
    // Actualizar estado a Rechazada y notificar
    $upd = $conexion->prepare("UPDATE solicitudConstanciaEstudiante SET estado = 'Rechazada' WHERE id = ?");
    $upd->bind_param("i", $solicitudId);
    $upd->execute();
    $upd->close();
    echo json_encode(['error' => true, 'mensaje' => 'El estudiante no cumple los requisitos: curso no aprobado']);
    exit();
}

// 4. Generar código único de constancia
...
$notaFinal      = number_format((float)$sol['notaFinal'], 2);
$resultado      = $sol['estadoEstudiante'];
$motivo         = $sol['motivo'];
$destino        = 'Estudiante';
$tipoConstancia = 'Constancia de aprobación de curso';
...
$fechaActividad = $sol['fechaActividad'] ? date('d/m/Y', strtotime($sol['fechaActividad'])) : $fechaSolicitud;
$codigoConstancia = $codigo;
$estado         = 'Generada';

// 6. Renderizar plantilla HTML
ob_start();
...
// 9. Insertar en tabla constancias
// Obtener usuario admin
$stmtAdmin = $conexion->prepare("SELECT u.id FROM usuarios u WHERE u.correo = ? LIMIT 1");
...
// 10. Actualizar estado de la solicitud a Aprobada
$upd = $conexion->prepare("UPDATE solicitudConstanciaEstudiante SET estado = 'Aprobada' WHERE id = ?");
$upd->bind_param("i", $solicitudId);
$upd->execute();
$upd->close();

// 11. Enviar PDF por correo al estudiante
...
try {
    ...
    $mail->Body    = "
        ...
        <p>Tu constancia de aprobación del curso <strong>{$curso}</strong> ha sido generada.</p>
        <p><strong>Código:</strong> {$codigo}<br>
           <strong>Nota final:</strong> {$notaFinal}<br>
    ...
    ";
    $mail->AltBody = "Hola {$solicitante}, tu constancia {$codigo} del curso {$curso} ha sido generada. Nota: {$notaFinal}. Revisa el archivo adjunto.";

    $mail->send();
} catch (Exception $e) {
    error_log('Error enviando constancia: ' . $e->getMessage());
